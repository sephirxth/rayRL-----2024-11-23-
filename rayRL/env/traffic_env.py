...
class TrafficSignalController:
    ...
    def __init__(self, sumo, ts_id, ts_lanes, num_phases, min_green, max_green, reward_fn=None):
        """
        初始化交通信号灯控制器

        参数:
            sumo: SUMO traci 连接实例
            ts_id (str): 交通信号灯的ID
            ts_lanes(str): 交通灯实际控制的车道ID列表
            num_phases (int): 信号灯的相位数量
            min_green (int): 绿灯的最短持续时间（秒）
            max_green (int): 绿灯的最长持续时间（秒）
        ...
        # 车道及其属性
        self.lanes = self.sumo.trafficlight.getControlledLanes(self.ts_id)  # 获取该信号灯控制的车道
        ...
        self.lane_lengths = {lane: self.sumo.lane.getLength(lane) for lane in self.filtered_lane} # 获取车道长度信息
    ...
    def get_state(self):
        ...
        """
        # 当前绿灯阶段的 one-hot 编码
        self.current_phase = self.sumo.trafficlight.getPhase(self.ts_id)
    ...
    def _get_lane_density(self, lane):
        """计算车道的密度（车辆数 / 车道容量）"""
        return min(1.0, self.sumo.lane.getLastStepVehicleNumber(lane) / self._get_lane_capacity(lane))

    def _get_lane_queue(self, lane):
        """计算车道的排队比例（排队车辆数 / 车道容量）"""
        return min(1.0, self.sumo.lane.getLastStepHaltingNumber(lane) / self._get_lane_capacity(lane))

    def _get_lane_capacity(self, lane):
        """计算车道的容量（能够容纳的最大车辆数）"""
        return self.lane_lengths[lane] / (2.5 + self.sumo.lane.getLastStepLength(lane))  # 假设车间距为2.5米
    ...
    def default_reward_fn(self):
        """默认奖励函数：负的所有车道的总等待时间"""
        waiting_times = [self.sumo.lane.getWaitingTime(lane) for lane in self.filtered_lane]
